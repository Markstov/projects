...
tableID = []
tableSLU = ["for", "to", "do", "while", "begin", "end", "var", "real", "string", "integer", "array", "of", "if", "then", "else","readln","writeln","read","goto","div","mod","program","downto","and","or","not","xor","procedure"]
tableCONST = []
tableR = [";", ",", ":", "(", ")",".","[", "]"]
tableO = ["+", "-", "*", "/", "^", "<", ">", "=", "<>", "<=", ">=", ":="]
tableC = []
...
def FuncC(string,res_str, check):
  ...
  for i in string[1:]:
    if i <= "9" and i >= "0" and check:
      ...
      tx += i
    elif i == "e" and check1 and not(check) and string[string.index(i) - 1] != ".":
      tx += i
      check1 = False
    elif string[string.index(i) - 1] == "e" and (i == "+" or i == "-"):
      tx += i
    elif i in tableO or i in tableR or i == " ":
...
def func_main(string,res_str):
  ...
  if len(string) != 0:
    if (string[0] >= 'a' and string[0] <= 'z') or string[0] == "_":
    ...
    elif (string[0] == ":" and len(string) == 1) or (string[0] == ":" and string[1] != "="):
